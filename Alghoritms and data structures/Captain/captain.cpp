...
int main()
{
    ...
    std::sort(islands.begin(), islands.end(),
              ...
              {
                  if (std::get<0>(lhs) != std::get<0>(rhs)) // sort by x...
                      return std::get<0>(lhs) < std::get<0>(rhs);
                  else
                      return std::get<1>(lhs) < std::get<1>(rhs); // then by y
              });

    // connect closest neighbours by x: (add numbers of x-neighbours to neighbour list
    std::get<3>(islands[0]).push_front((int &&) std::get<2>(islands[1]));
    for(int i = 1; i < n - 1; i++) {
        std::get<3>(islands[i]).push_front((int &&) std::get<2>(islands[i - 1]));
        std::get<3>(islands[i]).push_front((int &&) std::get<2>(islands[i + 1]));
    }
    std::get<3>(islands[n - 1]).push_front((int &&) std::get<2>(islands[n - 2]));

    // sort by y:
    std::sort(islands.begin(), islands.end(),
              [] (const island& lhs, const island & rhs)
              {
                  if (std::get<1>(lhs) != std::get<1>(rhs)) // sort by y...
                      return std::get<1>(lhs) < std::get<1>(rhs);
                  else
                      return std::get<0>(lhs) < std::get<0>(rhs); // then by x
              });
